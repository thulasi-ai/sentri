...
>
> **Current sprint:** `AUTO-017.3 + PROC-001` (bundled) — Web Vitals trend chart + backend-PRs-must-ship-UI convention (promoted per `NEXT.md` rotation after `CAP-004 + MET-001 + PROC-002` shipped in PR #14) · **Blockers:** none (MET-001 ✅ shipped in PR #14 — `<TrendChart>` + `metric_samples` available) · **Remaining:** 22 items (CAP-004 + MET-001 + PROC-002 ✅ shipped in PR #14 — self-healing telemetry dashboard at `/healing` with per-strategy success rate / top-healed selectors / savings trend, shared `metric_samples` time-series table + `recordMetric()` helper wired into `healingPersistence.js`, reusable `<TrendChart>` component consumed by the healing dashboard, sprint hand-off scaffold `scripts/promote-sprint-item.mjs` registered in `backend/tests/run-tests.js`; CAP-003 ✅ shipped in PR #12 — secret scanner gate on AI-generated Playwright tests; UI-REFACTOR-001 ✅ shipped in PR #6 — `ConfigurablePanel` abstraction + Automation page tabs + status-chip cache; combined recorder PR `DIF-015b Gap 3` + `DIF-015c Gap 1` ✅ shipped in PR #11 — iframe `frameLocator` emission, shadow-DOM via Playwright's InjectedScript, paste-as-single-`fill`, opt-in `shortcutCaptureBudget`; AUTO-019 ✅ shipped in PR #10; DIF-005 ✅ shipped in PR #9; AUTO-017 ✅ shipped in PR #8; DIF-015b Gap 2 ✅ shipped in PR #4; AUTO-012 ✅ shipped in PR #2; INF-006 ✅ shipped in PR #1; ENH-036 + ENH-036b ✅ shipped in PR #127; AUTO-016b ✅ shipped in PR #127; DIF-007 ✅ shipped in PR #123; MNT-006 ✅ shipped in PR #122)
