...
**Critical gaps to close next:** AUTO-004 (test impact analysis from git diff — current PR, builds on AUTO-001's shipped risk scorer and consumes INT-002's GitHub PR-files API path) · DIF-012 (multi-environment support — high enterprise-procurement value, pairs with shipped INT-002 for per-env check-name suffixes) · CAP-001 (data-driven testing — parameterized iterations, fills the largest remaining Phase 4 capability gap).
...
-->
**Total tracked items:** 99 across 9 categories — **57 complete** (58%), **1 in current PR** (AUTO-004), **41 remaining**
...
1. `AUTO-004` (current PR — test impact analysis from git diff, consumes INT-002's GitHub PR-files API path)
2. `INF-008` + `MNT-014` (bundled — Postgres default + migration linter)
3. `INF-007` + `MNT-013` (bundled — OTel/Sentry + request-ID propagation)
4. `SEC-004` (MFA — compliance unlock)
5. `AUTO-022` (eval harness — regression protection before any prompt change)
6. `SEC-006` (PII firewall — uses AUTO-022 golden set as test fixture)
7. `CAP-001` (data-driven testing — parameterized iterations)
8. `DIF-012` (multi-environment, pairs with shipped INT-002 for per-env check-name suffixes)
