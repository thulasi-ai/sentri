...
| New user-facing flow (auth, project CRUD, run lifecycle, export, etc.) | **UI E2E spec** exercising the real browser flow (Playwright `page` fixture, role-based selectors, runs under `--project=ui-chromium`) **and** a corresponding section / Golden E2E step in [QA.md](./QA.md) **and** a ✅ row in [`tests/e2e/COVERAGE.md`](./tests/e2e/COVERAGE.md). API-only specs are accepted as **fallback** only when the flow has no UI surface, the UI isn't shipped yet, or the UI flow is prohibitively flaky/slow — document the reason in the COVERAGE row and tag `(API-only)`. See [`tests/e2e/COVERAGE.md`](./tests/e2e/COVERAGE.md) § UI-first policy. | `tests/e2e/specs/<area>-ui.spec.mjs` for the UI spec (see STANDARDS.md § E2E Tests); update `QA.md` at repo root; flip the matching row in `tests/e2e/COVERAGE.md` from 🟥/🟨 to ✅ |
